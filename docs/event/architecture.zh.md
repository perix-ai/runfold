...
> 处理上游代码与依赖"。需求见 [`requirements.zh.md`](requirements.zh.md)，接口与
> 数据格式见 [`specification.zh.md`](specification.zh.md)，关键取舍的来龙去脉见
> [`decisions.zh.md`](decisions.zh.md)。
...
| Harness 宿主与插件机制 | 删除；影响生命周期的部分用最小本地接口重现 | Cordis Context/Service、scope、typert、插件注册 | 已由 `EventHost` 替换，见 [`decisions.zh.md`](decisions.zh.md) D01 |
...
`listSnapshots` 等）在 [`specification.zh.md`](specification.zh.md) 中逐项列出。
