...
| 层次 | 证明什么 | 位置 |
| --- | --- | --- |
| 文档配对 | 五份治理文档均有英文规范版和中文翻译版，语言链接有效、无孤儿文件，且最近一次修改来自同一提交 | `scripts/verify-doc-pairs.mjs` |
| 上游一致性 | 保留源码除登记的宿主接缝外与固定 commit 逐字节一致 | `scripts/verify-upstream-identity.mjs`，`npm run verify` 的首个上游检查 |
...
当前完整门禁先检查五组双语文档，再校验 204 个保留文件、10 个必要差异和
139 个声明映射，并运行 1005 个行为测试（626 Event、182 UI runtime、94
Trajectory、11 EventHost、15 SDK、33 UI、36 Python、1 系统、7 跨语言），
随后安装 TypeScript 与 Python 空白消费者。构建后还会扫描 Runfold 公共身份，
并由两个空白消费者检查发布包名称、内容及随包许可证。
