...
/**
 * Standalone-host port of the upstream `views.client.spec.tsx` cases that do
 * not need the complete DSH shell (docs/event/tasks.md R10). The retained
 * `TrajectoryView` and `TrajectoryTimeline` are rendered directly with the
 * session-scope kit the DSH outlet would bake, so every assertion below is
 * the upstream assertion; only the mounting differs. The six upstream cases
 * that exercise plugin registration, the Conversation tab ring, and the
 * Node-side plugin half are not ported: those are shell mechanisms the Event
...
import type { SessionId } from '@runfold/event/session/types'
...
import { TrajectoryTimeline as LocalizedTrajectoryTimeline } from '../../packages/client/ui-trajectory/src/client/TrajectoryTimeline.tsx'
...
/** The ledger's node vocabulary, as the Trajectory snapshot carries it. */
type ConversationNodes = TrajectorySnapshot['eventNodes']
...
function sessionSnapshot(nodes: ConversationNodes): SessionSnapshot {
  return {
    sessionId: SID,
    queue: [],
    pendingSubmissions: [],
...
/** Mount the retained view over one history snapshot with the standalone kit. */
function mountView(snapshot: TrajectorySnapshot) {
  return render(
    <TrajectoryView
      {...standaloneProps(snapshot.eventNodes)}
...
describe('ledger and inspector (standalone host)', () => {
  it('renders the event ledger with its toolbar, timeline, and turn collapse controls', () => {
    const view = mountView(historySnapshot(NODES))
  ...
  it('opens a local record inspector and switches payload tabs', () => {
    mountView(historySnapshot(NODES))
  ...
  it('labels a standalone compaction as between-turn work in the ledger and inspector', () => {
    ...
    const view = mountView(historySnapshot(nodes, { requests: [compaction] }))
  ...
  it('activates only the selected standalone compaction section', () => {
    ...
    mountView(historySnapshot(nodes, { requests: compactions }))
  ...
  it('clicking a timeline block clears the range, selects the record, and opens its inspector', () => {
    ...
    const toolSpan = view.container.querySelector<HTMLElement>('[data-timeline-span="tool"]')
    ...
    fireEvent.pointerDown(toolSpan as HTMLElement, { button: 0, clientX: 50, pointerId: 2 })
  ...
  it('empty window keeps the toolbar and reports no timing data', () => {
    mountView(historySnapshot([]))
    expect(screen.getByRole('toolbar', { name: '轨迹工具栏' })).toBeTruthy()
    expect(screen.getByText('无计时数据')).toBeTruthy()
    expect(screen.getByRole<HTMLButtonElement>('button', { name: '收起所有轮次' }).disabled).toBe(false)
    expect(screen.getByRole<HTMLButtonElement>('button', { name: '收起所有调用' }).disabled).toBe(false)
...
describe('timeline projection', () => {
  ...
  it('splits assistant time into recorded TTFT and decoding proportions with a delayed tooltip', () => {
    ...
    try {
      ...
      const span = view.container.querySelector<HTMLElement>('[data-timeline-span="message"]')
  ...
  it('marks an unloaded history prefix without inventing timeline duration', () => {
    ...
    expect(screen.getByRole('tooltip').textContent).toContain('Click to load earlier history')
  ...
  it('cancels native scrolling across the timeline while zooming', () => {
    ...
    expect(fireEvent.wheel(screen.getByText('Input'), { clientX: 20, deltaY: -100 })).toBe(false)
  })
  ...
  it('scales sequence gutters with narrow operation spans', () => {
    ...
    expect(span?.style.getPropertyValue('--trajectory-span-gap')).toBe('min(0.8%, 1px)')
  })
  ...
  it('keeps dense sequence spans proportional before applying the pixel floor', () => {
    ...
    expect(span?.style.getPropertyValue('--trajectory-span-gap')).toBe('min(0.02%, 1px)')
  })
  ...
  it('auto-pans a zoomed viewport while a range drag pushes against an edge', () => {
    ...
    const draftSelection = view.container.querySelectorAll<HTMLElement>('[data-dragging="true"]')
    expect(draftSelection).toHaveLength(2)
    for (const overlay of draftSelection) {
      expect(Number.parseFloat(overlay.style.getPropertyValue('--trajectory-selection-left'))).toBeLessThan(0)
    }
  ...
  it('uses equal-width operation slots and stable semantic lanes', () => {
    expect(deriveTrajectoryTimeline(turns)).toEqual({
      ...
      spans: [
        { index: 1, isError: false, kind: 'message', label: 'assistant', lane: 1, start: 0, end: 1 },
        { index: 2, isError: false, kind: 'tool', label: 'bash', lane: 2, start: 1, end: 2 },
        { index: 3, isError: false, kind: 'user', label: 'unknown', lane: 0, start: 2, end: 3 },
      ],
  ...
  it('marks error records directly on timeline spans', () => {
    const errorTurns = [{
      ...
      groups: [{
        title: 'Step 1',
        cells: [{ index: 1, kind: 'tool' as const, text: 'failed tool', timeSeconds: 0.1, isError: true }],
      }],
    }] satisfies readonly TrajectoryTurnModel[]
    const view = render(
      <TrajectoryTimeline
    ...
    expect(view.container.querySelector('[data-timeline-span="tool"][data-error="true"]')).toBeTruthy()
  })
  ...
  it('projects between-turn compaction without inventing a turn boundary', () => {
    const withStandaloneCompaction = [
      {
        turn: 1,
        groups: [{ title: 'Step 1', cells: [{ index: 1, kind: 'message', text: 'before', timeSeconds: 0 }] }],
      },
      {
        turn: null,
        groups: [{ title: 'Compaction 3', cells: [{ index: 2, kind: 'compacted', text: 'summary', timeSeconds: 0 }] }],
      },
      {
        turn: 2,
        groups: [{ title: 'Step 1', cells: [{ index: 3, kind: 'message', text: 'after', timeSeconds: 0 }] }],
      },
    ] satisfies readonly TrajectoryTurnModel[]
...
describe('TrajectoryView state', () => {
  ...
  it('keeps ledger and timeline selection on the same event after prepend', () => {
    const older: ConversationNodes[number] = {
      kind: 'user', seq: 1, time: 1_000,
      content: [{ type: 'text', text: 'older prompt' }], source: null,
    }
    const current: ConversationNodes[number] = {
